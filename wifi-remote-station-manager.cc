    ...
    m_ampdu_start_seq(0)
{
  ...
  initial_m_retry_sequence = 0;
  remember_stage = 0;
  count_total_data_frames = 0;
  count_total_retry = 0;
  remember_after_RTS_sequence.push_back(-1);
  remember_p0_seq=0;
  remember_p1_seq = 0;
  remember_p2_seq = 0;
  count_after_RTS_retry = 0;
  count_after_RTS_times = 0;
  count_after_RTS_total = 0;
  count_total_rts = 0;
  count_total_rts_loss = 0;
  er_after_RTS = 0;
  er_total = 0;
  er_RTS = 0;
...
uint16_t 
WifiRemoteStationManager::Get_stage ()
{
        return remember_stage;
}
void
                                            ...
                                            uint32_t packetSize)
{
  ...
  uint16_t sequenceNumber_datafa;
  sequenceNumber_datafa = header->GetSequenceNumber();
        
  int len_remember_sft_rts_seq = remember_after_RTS_sequence.size();
  for (int i = len_remember_sft_rts_seq-1; i>0;i--)
  {
        if (remember_after_RTS_sequence.at(i) == sequenceNumber_datafa)
        {

                count_after_RTS_retry = count_after_RTS_retry+1; //when this sequence of data frame retry, retry after RTS plus 1
                count_after_RTS_total = count_after_RTS_total+1;
                remember_after_RTS_sequence.erase( remember_after_RTS_sequence.begin() + i );
                return;
        }
    }
count_total_data_frames = count_total_data_frames+1;
count_total_retry = count_total_retry+1;
                                        ...
                                        WifiTxVector dataTxVector, uint32_t packetSize)
{
  ...
  uint16_t sequenceNumber_dataok;
  sequenceNumber_dataok = header->GetSequenceNumber();

  int len_remember_sft_rts_seq = remember_after_RTS_sequence.size();
  for (int i = len_remember_sft_rts_seq-1; i>0;i--)
  {

        if (remember_after_RTS_sequence.at(i) == sequenceNumber_dataok)
        {
                count_after_RTS_total = count_after_RTS_total+1;
                remember_after_RTS_sequence.erase( remember_after_RTS_sequence.begin() + i );
        return;
...
void
WifiRemoteStationManager::ReportMissRts (Mac48Address address, const WifiMacHeader *header)
{
  count_total_rts_loss = count_total_rts_loss+1;
}
...
void
WifiRemoteStationManager::ReportAmpduTxStatus (uint16_t m_ampdu_start_seq,Mac48Address address,
                                               ...
                                               double rxSnr, double dataSnr, WifiTxVector dataTxVector)
{
  ...
  for (int i = len_remember_sft_rts_seq-1; i>0;i--)
  {
        ...
        if (remember_after_RTS_sequence.at(i) == m_ampdu_start_seq)
        {
                count_after_RTS_total = count_after_RTS_total + nSuccessfulMpdus + nFailedMpdus;
                count_after_RTS_retry = count_after_RTS_retry+nFailedMpdus; //when this sequence of data frame retry, retry after RTS plus 1
                remember_after_RTS_sequence.erase( remember_after_RTS_sequence.begin() + i );
  ...
  count_total_data_frames = count_total_data_frames+nSuccessfulMpdus+nFailedMpdus; // it is actually the unprotected frames
  count_total_retry = count_total_retry+nFailedMpdus;   

}

bool
WifiRemoteStationManager::isAccessNeedRts (const WifiMacHeader &header, uint32_t size)
{
...
void
WifiRemoteStationManager::setAmpdustartse (uint32_t seq)
{       
        m_ampdu_start_seq = seq;
}
...
WifiRemoteStationManager::NeedRts (const WifiMacHeader &header, uint32_t size)
{
  ...
  //change this part to be when retransmitting a frame use RTS no matter how small the size of this frame is.
//  bool normally = (size > m_rtsCtsThreshold);
  bool isData = header.IsData();
  int lambda_rts = 600; // influence accuracy and speed
  int lambda_total = lambda_rts; //influence accuracy and speed
  double alpha_total = 0.03;
  double alpha_rts = 0.03; // gamma
  double beta = 0.4;
  double p1 = 0.8;
  double p2 = 0.4;
  double omega = 1;
  ...
  if (count_total_data_frames>lambda_total)  
  {                               
          er_total = ((double)(count_total_retry/count_total_data_frames));
          er_total = (1-omega)*er_total + omega*(count_total_retry/count_total_data_frames);

          if (er_total<=alpha_total)
          {                
                ...
                count_total_rts = 0;
                count_total_rts_loss = 0;
//                std::cout<<"stage 2,  er total"<<er_total<<std::endl;

          }
          else if (count_after_RTS_total > lambda_rts) 
          {


            er_RTS = (1-omega)*er_RTS + omega*(count_total_rts_loss/count_total_rts);

            if (count_total_retry!=count_after_RTS_retry)
            {
                    er_after_RTS= (1-omega)*er_after_RTS + omega*((double)(count_after_RTS_retry/count_after_RTS_total));


                    if ((er_total>alpha_total and er_after_RTS<=er_total*beta) or er_RTS>alpha_rts)
                    {
                            remember_stage = 1;
  ...
  if (isData)
  {
    ...
    bool normally = header.IsRetry();
    uint16_t sequenceNumber_need;
    sequenceNumber_need = header.GetSequenceNumber();
    NS_LOG_FUNCTION("Destination addr"<<header.GetAddr1 ()<<"Transmitter addr"<<header.GetAddr2 ());
    if(remember_stage == 0)   // initial stage, use RTS/CTS for 50% first retransmission
    {
        double random_r;
        bool if_RTS;
        ...
        //sequence
        if (remember_p0_seq<1)
        {
                if_RTS = true;
                remember_p0_seq = remember_p0_seq+1;
                remember_after_RTS_sequence.push_back(sequenceNumber_need); //remember the sequence of the data frame after RTS/CTS    just after RTS
                count_total_rts = count_total_rts+1;
                bool re = DoNeedRts (Lookup (address), size, if_RTS);
               return re;

        }
        else if (remember_p0_seq>=1 and remember_p0_seq<2)
        {
               remember_p0_seq = remember_p0_seq+1;
               if (remember_p0_seq == 2)
               {
                        remember_p0_seq = 0;
                }
     ...
     else if (remember_stage == 1) // high collision stage, use RTS/CTS for every first retransmission
     {
            double random_r;
            bool if_RTS;
            //sequence

            if (remember_p1_seq<p1*10)
            {
                    if_RTS = true;
//                                std::cout<<"remember_p1_seq"<<remember_p1_seq<<std::endl; 
                    remember_p1_seq = remember_p1_seq+1;
                    remember_after_RTS_sequence.push_back(sequenceNumber_need); //remember the sequence of the data frame after RTS/CTS    just after RTS
                    count_total_rts = count_total_rts+1;
                    bool re = DoNeedRts (Lookup (address), size, if_RTS);
                    return re;
            ...
            else if (remember_p1_seq>=p1*10 and remember_p1_seq<=10)
            {

                   if (remember_p1_seq == 10)
                   {
                            remember_p1_seq = 0;
                   }
                    else
     ...
     //remember_p1_seq
     else if (remember_stage == 2) // no/low collision, use 40% RTS/CTS for the first transmission
     {
            bool if_RTS;
            double random_r2;
            ...
            // sequence
            if (remember_p2_seq<p2*10)
            {
                    if_RTS = true;
                    remember_p2_seq = remember_p2_seq+1;
                    remember_after_RTS_sequence.push_back(sequenceNumber_need); //remember the sequence of the data frame after RTS/CTS    just after RTS
                    count_total_rts = count_total_rts+1;
                    bool re = DoNeedRts (Lookup (address), size, if_RTS);
                    return re;//return re;
           ...
           else if (remember_p2_seq>=p2*10 and remember_p2_seq<=10)
           {

                   if (remember_p2_seq == 10)
                    {
                            remember_p2_seq = 0;
                    }
                    else
