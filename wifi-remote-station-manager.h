...
public:
  ...
  void ReportMissRts (Mac48Address address, const WifiMacHeader *header);
  /**
   ...
   */
  void ReportAmpduTxStatus (uint16_t m_ampdu_start_seq, Mac48Address address, uint8_t nSuccessfulMpdus, uint8_t nFailedMpdus,
   ...
   */
  void setAmpdustartse (uint32_t seq);
  bool isAccessNeedRts (const WifiMacHeader &header, uint32_t size);
...
private:
  ...
  uint16_t initial_m_retry_sequence;
  std::vector<uint16_t> remember_after_RTS_sequence;
  uint16_t remember_p1_seq;
  uint16_t remember_p2_seq;
  uint16_t remember_p0_seq;
  double er_after_RTS;
  double er_total;
  double er_RTS;
  uint16_t remember_stage;  // 0 for initial status, 1 for high collision status, 2 for high poor SNR with no/low collision ----start from initial stage with 0
  double count_total_data_frames;
  double count_total_retry;
  double count_after_RTS_total;
  double count_after_RTS_retry;
  double count_after_RTS_times;
  double count_total_rts;
  double count_total_rts_loss;
  uint32_t m_ampdu_start_seq;
